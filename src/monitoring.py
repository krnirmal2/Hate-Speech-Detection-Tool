...
TWEETS_BY_RISK = Counter(
    'tweets_by_risk_total',
    'Number of tweets by risk level',
    ['risk_level']
)

PROCESSING_TIME = Histogram(
    'tweet_processing_seconds',
    'Time spent processing tweets'
)

SENTIMENT_SCORE = Gauge(
    'tweet_sentiment_score',
    'Sentiment score of processed tweets'
)

CENTRALITY_SCORE = Gauge(
    'user_centrality_score',
...
def update_metrics(tweet_data: dict):
    ...
    CENTRALITY_SCORE.labels(
        username=tweet_data.get('username', 'unknown')
    ).set(tweet_data.get('centrality_score', 0)) 