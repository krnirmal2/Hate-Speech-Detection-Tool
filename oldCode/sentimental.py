...
for i, (tweet, sentiment) in enumerate(
    zip(data["Cleaned_Tweet"][:5], sentiment_categories[:5]), start=1
):
    print(f"Tweet {i}:")
    print(f"Text: {tweet}")
...
sentiment_features = np.array(
    [score["compound"] for score in sentiment_scores]
).reshape(-1, 1)
...
users = data[
    "username"
].unique()  # Assuming 'User' is the user identifier in your dataset
...
nx.draw_networkx_nodes(
    G, pos, nodelist=[most_central_user], node_color="red", node_size=3000
)
